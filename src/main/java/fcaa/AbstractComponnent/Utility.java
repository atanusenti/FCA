...
public class Utility {
	...
	public List<String> titleList(List<WebElement> elements) throws InterruptedException {
		...
		while (!elements.isEmpty()) {
			...
			if (nextPageButton.isEnabled() && nextPageButton.isDisplayed()) {
				...
				Reporter.log("Next Page button is disabled or not found");
	...
	public void changeSortingValues() {
		try {
			...
			// Wait for the sorting option to be visible
			WebElement sortOption = waitForElementToBeVisible(By.xpath("//li[@aria-label='100']"));

			// Click the sort option to apply the sort
			sortOption.click();
	...
	@FindBy(xpath = "//div[contains(@class,'flex align-items-center gap-2 md:gap-4 flex-column sm:flex-row mt-3 ng-star-inserted')]//div[contains(class,'toolbar text-sm py-2 px-2 border-round-lg flex-1')]")
